...
const List = (props) => {
    ...
    const updateDatbase = (payload) => {
        fetch(`${process.env.REACT_APP_ALPHA_SERVER}/updateField`,{ 
            method: "POST",
            headers:{
    ...
    const deleteList = (e) => {
        ...
        fetch(`${process.env.REACT_APP_ALPHA_SERVER}/deleteData`,{ 
            method: "DELETE",
            headers:{
                'Content-type': 'application/json'
            },
    ...
    return(
        <div className={styles.list} id={key} >
            ...
            <div className={styles.pin}>
                {
                    pinned ?
                    <img alt = "pin" src="./icons/pin2.svg" /> : ""
                }
            </div>
                ...
                <div className={styles.menuList} ref={menu} data-status="close">
                    <ul>
                        <li onClick={pin}
                            ...
                            <img alt = "pin" src="./icons/pin.svg"/>
                            {pinned ? "Unpin" : "Pin"}
                        </li>
                        <li onClick={deleteList} >
                            <img alt = "delete" src="./icons/deleteIcon.svg" />
                            Delete 
                        </li>
                    </ul>
                </div>
                <img alt = "..." onClick={() => props.checkForToggleClearance(menu)} src="./icons/three-dots.svg" />
            </div>
        </div>
    );
}
